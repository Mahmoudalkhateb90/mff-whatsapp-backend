...
export default function BulkBroadcast() {
  ...
  const fetchActiveCampaign = async () => {
    ...
    const activeUserId = currentUser?.id || currentUser?.uid || userId || localStorage.getItem('user_id');
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/campaigns/active`, {
        headers: {
          ...
          'Authorization': `Bearer ${localStorage.getItem('auth_token') || activeUserId}`
        }
      });
      if (res.ok) {
